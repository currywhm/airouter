...
func (r *affiliateRepository) GetDailyCheckinStatus(ctx context.Context, userID int64, checkinDate string) (*service.DailyCheckinStatus, error) {
	client := clientFromContext(ctx, r.client)
	rows, err := client.QueryContext(ctx, `
SELECT $2::text,
       EXISTS (SELECT 1 FROM daily_checkins WHERE user_id = $1 AND checkin_date = $2::date),
       COALESCE((SELECT reward_amount FROM daily_checkins WHERE user_id = $1 AND checkin_date = $2::date), 0),
       u.balance
FROM users u
WHERE u.id = $1 AND u.deleted_at IS NULL`, userID, checkinDate)
	if err != nil {
	...
	if err := rows.Scan(&status.Date, &status.Claimed, &status.Reward, &status.Balance); err != nil {
		return nil, fmt.Errorf("scan daily check-in status: %w", err)
	}
	return status, nil
}

func (r *affiliateRepository) ClaimDailyCheckin(ctx context.Context, userID int64, checkinDate string, reward float64) (*service.DailyCheckinStatus, error) {
	if userID <= 0 || reward < 1 || reward > 3 {
		return nil, service.ErrUserNotFound
	}
	if tx := dbent.TxFromContext(ctx); tx != nil {
		return claimDailyCheckinTx(ctx, tx.Client(), userID, checkinDate, reward)
	}
	...
	status, err := claimDailyCheckinTx(dbent.NewTxContext(ctx, tx), tx.Client(), userID, checkinDate, reward)
	if err != nil {
		return nil, err
	}
...
func claimDailyCheckinTx(ctx context.Context, client affiliateQueryExecer, userID int64, checkinDate string, reward float64) (*service.DailyCheckinStatus, error) {
	rows, err := client.QueryContext(ctx, `
INSERT INTO daily_checkins (user_id, checkin_date, reward_amount, created_at)
VALUES ($1, $2::date, $3, NOW())
ON CONFLICT (user_id, checkin_date) DO NOTHING
RETURNING reward_amount`, userID, checkinDate, reward)
	if err != nil {
...
WHERE id = $2 AND deleted_at IS NULL`, reward, userID)
		if err != nil {
	...
	status := &service.DailyCheckinStatus{}
	rows, err = client.QueryContext(ctx, `
SELECT $2::text, TRUE, reward_amount, u.balance
FROM daily_checkins dc
JOIN users u ON u.id = dc.user_id
WHERE dc.user_id = $1 AND dc.checkin_date = $2::date AND u.deleted_at IS NULL`, userID, checkinDate)
	if err != nil {
		return nil, fmt.Errorf("read daily check-in result: %w", err)
	}
	defer func() { _ = rows.Close() }()
	if !rows.Next() {
		if err := rows.Err(); err != nil {
