...
// GetDailyCheckinStatus returns the current user's claim state for today.
func (h *UserHandler) GetDailyCheckinStatus(c *gin.Context) {
	...
	status, err := h.affiliateService.GetDailyCheckinStatus(c.Request.Context(), subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
...
// ClaimDailyCheckin claims today's random $1-$3 wallet reward.
func (h *UserHandler) ClaimDailyCheckin(c *gin.Context) {
	...
	status, err := h.affiliateService.ClaimDailyCheckin(c.Request.Context(), subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
