...
// DailyCheckinRepository is kept separate from AffiliateRepository so the
// check-in feature does not widen the interface implemented by existing test
// doubles and affiliate integrations.
type DailyCheckinRepository interface {
	GetDailyCheckinStatus(ctx context.Context, userID int64, checkinDate string) (*DailyCheckinStatus, error)
	ClaimDailyCheckin(ctx context.Context, userID int64, checkinDate string, reward float64) (*DailyCheckinStatus, error)
}
...
// GetDailyCheckinStatus returns today's claim state. The date is generated in
// the server's configured timezone, matching the platform's displayed day.
func (s *AffiliateService) GetDailyCheckinStatus(ctx context.Context, userID int64) (*DailyCheckinStatus, error) {
	repo, err := s.dailyCheckinRepo()
	if err != nil {
		return nil, err
	}
	return repo.GetDailyCheckinStatus(ctx, userID, time.Now().Format("2006-01-02"))
}

// ClaimDailyCheckin awards a random amount from $1.0 to $3.0 in $0.1 steps. The database
// unique constraint owns idempotency; a duplicate request returns the first
// claim rather than generating another reward.
func (s *AffiliateService) ClaimDailyCheckin(ctx context.Context, userID int64) (*DailyCheckinStatus, error) {
	repo, err := s.dailyCheckinRepo()
	if err != nil {
		return nil, err
	}
	reward, err := randomDailyCheckinReward()
	if err != nil {
		return nil, fmt.Errorf("generate daily check-in reward: %w", err)
	}
	status, err := repo.ClaimDailyCheckin(ctx, userID, time.Now().Format("2006-01-02"), reward)
	if err != nil {
		return nil, err
	}
	if status != nil {
...
func (s *AffiliateService) dailyCheckinRepo() (DailyCheckinRepository, error) {
	if s == nil || s.repo == nil {
		return nil, infraerrors.ServiceUnavailable("SERVICE_UNAVAILABLE", "daily check-in service unavailable")
	}
	repo, ok := s.repo.(DailyCheckinRepository)
	if !ok {
		return nil, infraerrors.ServiceUnavailable("SERVICE_UNAVAILABLE", "daily check-in service unavailable")
	}
	return repo, nil
