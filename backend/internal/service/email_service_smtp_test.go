...
func TestSMTPAuthMechanismPrefersLogin(t *testing.T) {
	if got := smtpAuthMechanism("LOGIN XOAUTH2"); got != "LOGIN" {
		t.Fatalf("expected LOGIN for Microsoft 365 capabilities, got %q", got)
	}
	if got := smtpAuthMechanism("PLAIN LOGIN"); got != "LOGIN" {
		t.Fatalf("expected LOGIN when both mechanisms are available, got %q", got)
	}
	if got := smtpAuthMechanism("PLAIN"); got != "PLAIN" {
