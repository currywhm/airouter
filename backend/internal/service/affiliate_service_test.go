...
func TestRandomDailyCheckinReward(t *testing.T) {
	t.Parallel()
	for range 200 {
		reward, err := randomDailyCheckinReward()
		require.NoError(t, err)
		require.GreaterOrEqual(t, reward, 1.0)
		require.LessOrEqual(t, reward, 3.0)
		require.Equal(t, reward, math.Round(reward*10)/10)
	}
}
