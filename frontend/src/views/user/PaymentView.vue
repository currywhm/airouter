        ...
        <template v-else>
          ...
          <template v-if="activeTab === 'recharge'">
            <div class="grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
              <section class="card overflow-hidden border-primary-100 bg-gradient-to-br from-primary-50 via-white to-white p-6 dark:border-primary-900/40 dark:from-primary-950/40 dark:via-dark-900 dark:to-dark-900 sm:p-8">
                <div class="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    ...
                    <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">{{ t('wallet.overview.subtitle') }}</p>
                    <p class="mt-6 text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-dark-500">{{ t('wallet.overview.availableBalance') }}</p>
                    <p class="mt-1 text-4xl font-semibold tracking-tight text-gray-950 dark:text-white sm:text-5xl">
                      ${{ user?.balance?.toFixed(2) || '0.00' }}
                    </p>
                  </div>
                  <div class="space-y-3 sm:text-right">
                    <p class="text-sm text-gray-500 dark:text-dark-400">{{ t('wallet.overview.account') }}</p>
                    <p class="max-w-[16rem] truncate text-sm font-semibold text-gray-900 dark:text-white sm:ml-auto">{{ user?.username || user?.email || '' }}</p>
                    <p class="flex items-center gap-1.5 text-xs text-gray-500 dark:text-dark-400 sm:justify-end">
                      <Icon name="checkCircle" size="sm" class="text-emerald-500" />
                      {{ t('wallet.overview.noSubscription') }}
                    </p>
                  </div>
                </div>
              </section>

              <section class="card border-amber-100 bg-amber-50/60 p-6 dark:border-amber-900/30 dark:bg-amber-950/20 sm:p-8">
                <div class="flex items-start justify-between gap-3">
                  <div>
                    <p class="flex items-center gap-2 text-base font-semibold text-gray-900 dark:text-white">
                      <Icon name="gift" size="md" class="text-amber-500" />
                      {{ t('wallet.checkin.title') }}
                    </p>
                  </div>
                  <span v-if="checkinStatus?.claimed" class="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-medium text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">
                    {{ t('wallet.checkin.claimed') }}
                  </span>
                </div>
                <div class="mt-6 flex items-end justify-between gap-4">
                  <div>
                    <p class="text-xs text-gray-500 dark:text-dark-400">{{ checkinStatus?.claimed ? t('wallet.checkin.reward') : t('wallet.checkin.claim') }}</p>
                    <p v-if="checkinStatus?.claimed" class="mt-1 text-3xl font-semibold text-amber-600 dark:text-amber-300">+${{ checkinStatus.reward.toFixed(1) }}</p>
                  </div>
                  <button class="btn btn-primary shrink-0" :disabled="checkinLoading || checkinStatus?.claimed" @click="claimCheckin">
                    <Icon v-if="checkinLoading" name="refresh" size="sm" class="animate-spin" />
                    <Icon v-else-if="checkinStatus?.claimed" name="check" size="sm" />
                    <Icon v-else name="gift" size="sm" />
                    <span>{{ checkinStatus?.claimed ? t('wallet.checkin.claimed') : t('wallet.checkin.claim') }}</span>
                  </button>
                </div>
...
async function loadCheckinStatus(): Promise<void> {
  try {
    checkinStatus.value = await userAPI.getDailyCheckinStatus()
  } catch (error) {
    appStore.showError(extractApiErrorMessage(error, t('wallet.checkin.loadFailed')))
  }
}

async function claimCheckin(): Promise<void> {
  if (checkinLoading.value || checkinStatus.value?.claimed) return
  checkinLoading.value = true
  try {
    const result = await userAPI.claimDailyCheckin()
    checkinStatus.value = result
    await authStore.refreshUser()
    appStore.showSuccess(`${t('wallet.checkin.balanceUpdated')}: +$${result.reward.toFixed(1)}`)
  } catch (error) {
    appStore.showError(extractApiErrorMessage(error, t('wallet.checkin.claimFailed')))
