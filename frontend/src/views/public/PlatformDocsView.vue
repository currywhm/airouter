            ...
            <table class="w-full min-w-[620px] text-left text-sm">
              <thead class="border-b border-gray-200 bg-gray-50 text-xs uppercase tracking-wide text-gray-500 dark:border-dark-800 dark:bg-dark-900/70 dark:text-dark-400">
                <tr>
                  ...
                  <th class="px-4 py-3 font-medium">{{ copy.cacheInputPrice }}</th>
              ...
              <tbody class="divide-y divide-gray-100 dark:divide-dark-800">
                <tr v-for="model in models" :key="model.name">
                  ...
                  <td class="px-4 py-3 font-mono text-gray-700 dark:text-dark-200">{{ model.cacheInput }}</td>
...
const copy = computed(() => isZh.value ? {
  ...
  walletTitle: '钱包与计费', walletDescription: '注册奖励和充值余额统一显示在钱包中，调用产生的费用从可用余额扣除。',
  modelsTitle: '模型与价格', modelsDescription: '价格按每百万 Token 展示，输入、缓存输入和输出分别计费。', model: '模型', positioning: '定位', inputPrice: '输入 / 1M', cacheInputPrice: '缓存输入 / 1M', outputPrice: '输出 / 1M', priceNote: '实际扣费以平台实时价格及请求产生的 Token 用量为准。输入、缓存输入、输出分别计费；缓存命中后按照缓存价格计费。',
  ...
  walletPoints: [
    { number: '01', title: '余额统一管理', body: '注册奖励和充值金额都在钱包中查看。' },
  ...
  walletTitle: 'Wallet and billing', walletDescription: 'Registration credit and top-ups are shown in the same wallet. Usage is deducted from the available balance.',
  modelsTitle: 'Models and pricing', modelsDescription: 'Prices are shown per million tokens. Input, cached input, and output are billed separately.', model: 'Model', positioning: 'Positioning', inputPrice: 'Input / 1M', cacheInputPrice: 'Cached input / 1M', outputPrice: 'Output / 1M', priceNote: 'Actual charges follow the real-time platform price and the token usage generated by each request. Input, cached input, and output are billed separately; cache hits use the cached-input price.',
  ...
  walletPoints: [
    { number: '01', title: 'One balance', body: 'Registration credit and top-ups are visible in Wallet.' },
